...
Тесты находятся в модуле `url-shortener-core`.
...
- один и тот же URL у разных пользователей дает разные короткие ссылки
- лимит переходов блокирует доступ
- истекшие ссылки удаляются при обращении и через cleanup
